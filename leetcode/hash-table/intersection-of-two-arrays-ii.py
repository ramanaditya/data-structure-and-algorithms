...
class Solution:
    ...
    def intersect(self, nums1: List[int], nums2: List[int]) -> List[int]:
        ...
        for k, v in d1.items():
            if d2.get(k):
                res.extend([k] * min(d1.get(k), d2.get(k)))

        return res


# Runtime: 40 ms, faster than 94.43% of Python3 online submissions
# Memory Usage: 14.3 MB, less than 88.88% of Python3 online submissions
